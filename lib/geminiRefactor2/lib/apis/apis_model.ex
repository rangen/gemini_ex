defmodule Gemini.APIs.Models do
  ...
  ## Examples
      ...
      # Handle not found
      case Models.get("invalid-model") do
  ...
  def get(model_name) do
    ...
    with {:ok, request} <- GetModelRequest.new(model_name),
         telemetry_opts = build_telemetry_opts(:get_model, model: extract_base_model_id(request.name)),
  ...
  defp model_matches_filter?(model, filter_opts) do
    Enum.all?(filter_opts, fn {key, value} ->
      case key do
        ...
        :production_ready -> (value and Model.production_ready?(model)) or (not value)
        :model_family -> Model.model_family(model) == value
        :latest_version -> (value and Model.is_latest_version?(model)) or (not value)
        _ -> true
      end
    end)
  end
