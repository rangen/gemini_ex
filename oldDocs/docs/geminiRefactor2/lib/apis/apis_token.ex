defmodule Gemini.APIs.Tokens do
  ...
  ## Examples
      ...
      # Count tokens for the complete request
      {:ok, response} = Tokens.count_for_request(gen_request)
  ...
  ## Examples
      ...
      |> Enum.each(fn {input, result} ->
        IO.puts("'#{input}' -> #{result.total_tokens} tokens")
      end)
  ...
  defp estimate_tokens_heuristic(content) when is_binary(content) do
    ...
    word_estimate = round(word_count * 1.3)  # ~1.3 tokens per word
    char_estimate = round(char_count / 4.0)  # ~4 chars per token
